"""
WEEK 5 — Speech-Text Representation (INSPIRED)
Goal:
...
from config_inspired import OUTPUT_DIR


TRAIN_META = os.path.join(OUTPUT_DIR, "metadata_with_acoustic_emotion_inspired.csv")
REPORT_FILE = os.path.join(OUTPUT_DIR, "week5_inspired_report.txt")
MODEL_PATH = os.path.join(OUTPUT_DIR, "week5_inspired_encoder.joblib")
...
class DualAligner(nn.Module):
    def __init__(self, text_dim, audio_dim, proj_dim=256):
        ...
        # Reconstruction heads for masked prediction
...
def masked_prediction_loss(original_emb, projected_emb, reconstructed_emb, mask_ratio=0.15):
    ...
    Similar to BERT-style masked language modeling
    """
...
def week5_pipeline(epochs=8, batch_size=32, proj_dim=256, contrastive_weight=1.0, masked_weight=0.5):
    ...
    print("\n========= WEEK 5 (INSPIRED): SPEECH-TEXT REPRESENTATION =========\n")
    print("[W5-INSPIRED] Training with Contrastive + Masked Prediction Objectives")
    ...
    print(f"[W5-INSPIRED] Loaded training metadata -> {len(df)} samples")
    ...
    # TEXT embeddings
    # ------------------------------------------------
    print("[W5-INSPIRED] Generating text embeddings...")
    ...
    # AUDIO embeddings
    # ------------------------------------------------
    print("[W5-INSPIRED] Generating audio embeddings...")
    audio_emb = []
    ...
    # ------------------------------------------------
    print(f"\n[W5-INSPIRED] Training for {epochs} epochs...")
    print(f"[W5-INSPIRED] Loss weights: Contrastive={contrastive_weight}, Masked={masked_weight}\n")
    ...
    print(f"\n[W5-INSPIRED] Encoder model saved -> {MODEL_PATH}")
    ...
    print(f"[W5-INSPIRED] Train embeddings saved -> {TRAIN_EMB_PATH}")
    print(f"[W5-INSPIRED] Embedding shape: {combined_emb.shape}")
    print(f"[W5-INSPIRED] Columns: {len(text_col_names)} text + {len(audio_col_names)} audio = {len(all_col_names)} total")

    # Summary statistics
    print(f"\n[W5-INSPIRED] ===== TRAINING SUMMARY =====")
    print(f"[W5-INSPIRED] Dataset size: {len(df)} samples")
    print(f"[W5-INSPIRED] Text embedding dim: {text_dim}")
    print(f"[W5-INSPIRED] Audio embedding dim: {audio_dim}")
    print(f"[W5-INSPIRED] Projection dim: {proj_dim}")
    print(f"[W5-INSPIRED] Training epochs: {epochs}")
    print(f"[W5-INSPIRED] Batch size: {batch_size}")
    print(f"[W5-INSPIRED] Objectives: Contrastive Learning + Masked Prediction")
    ...
    print(f"[W5-INSPIRED] Training logs saved -> {REPORT_FILE}")
    print(f"[W5-INSPIRED] Week 5 pipeline completed successfully!")
